...
def insert_hotelbeds_mapping_row(ittid, hotelbeds_hotel_id, country_code):
    sql = text(
        f"""
        INSERT INTO {table_3} (
            ittid,
...
def ensure_hotelbeds_mapping(hotelbeds_hotel_id, ittid, hotelbeds_mapping_cache, country_code):
    existing = hotelbeds_mapping_cache.get(hotelbeds_hotel_id)
    if existing:
        return existing

    mapping_id = insert_hotelbeds_mapping_row(ittid, hotelbeds_hotel_id, country_code)
    record = {
        "Id": mapping_id,
        "ittid": ittid,
        "supplier": SUPPLIER_NAME,
...
def update_mapping(mapping_id, ittid, best_score, analysis_payload, country_code):
    sql = text(
        f"""
        UPDATE {table_3}
        SET
...
def process_one_hotelbeds(
    ...
    if hb["lat"] is None or hb["lon"] is None:
        ...
        if existing:
            ...
            mapping = ensure_hotelbeds_mapping(
                hb["hotel_id"], new_ittid, hotelbeds_mapping_cache, hb["country_code"]
            )
            analysis_payload = build_analysis_payload(
                hb,
                SUPPLIER_NAME,
            ...
            update_mapping(
                mapping["Id"], new_ittid, get_zero_score_payload(), analysis_payload, hb["country_code"]
            )
    ...
    if not best_score:
        ...
        if existing:
            ...
            existing = ensure_hotelbeds_mapping(
                hb["hotel_id"], ittid, hotelbeds_mapping_cache, hb["country_code"]
            )
        ...
        update_mapping(
            existing["Id"], ittid, get_zero_score_payload(), analysis_payload, hb["country_code"]
        )
        update_hotelbeds_status(hb["Id"], "new-mapping")
    ...
    if best_score["total_bm"] >= AUTO_MATCH_THRESHOLD:
        ...
        mapping = ensure_hotelbeds_mapping(
            hb["hotel_id"], matched_ittid, hotelbeds_mapping_cache, hb["country_code"]
        )
        analysis_payload = build_analysis_payload(
            hb,
            SUPPLIER_NAME,
        ...
        update_mapping(mapping["Id"], matched_ittid, best_score, analysis_payload, hb["country_code"])
    ...
    if best_score["total_bm"] >= REVIEW_THRESHOLD:
        ...
        update_mapping(existing["Id"], ittid, best_score, analysis_payload, hb["country_code"])
    ...
    update_mapping(existing["Id"], ittid, get_zero_score_payload(), analysis_payload, hb["country_code"])
    update_hotelbeds_status(hb["Id"], "new-mapping")
