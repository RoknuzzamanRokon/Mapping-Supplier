...
def build_analysis_payload(
    ...
    for candidate in sorted_candidate_scores(candidate_scores)[:5]:
...
def process_one_hotelbeds(
    ...
    if hb["lat"] is None or hb["lon"] is None:
        ...
        print(f"SKIP: Hotelbeds {hb['hotel_id']} (no lat/lon)")
        return
    ...
    if not best_score:
        ...
        update_hotelbeds_status(hb["Id"], "new-mapping")
        print(f"❌ No candidates HB:{hb['hotel_id']} | new ittid:{ittid}")
        return

    # Collision check
    agoda_existing = agoda_mapping_cache.get(best_score["agoda_hotel_id"])
    if agoda_existing:
        existing_hb = hotelbeds_mapping_cache.get(agoda_existing["hotel_id"])
...
def fetch_single_hotelbeds_hotel(hotel_id):
    """Fetch a single Hotelbeds hotel by hotel_id"""
    sql = text(
        f"""
        SELECT
            Id,
...
def map_single_hotelbeds_hotel(hotelbeds_hotel_id):
    """Map a single Hotelbeds hotel to Agoda"""
    print(f"Loading single Hotelbeds hotel: {hotelbeds_hotel_id}")

    hb = fetch_single_hotelbeds_hotel(hotelbeds_hotel_id)
    if not hb:
        print(f"❌ Hotelbeds hotel {hotelbeds_hotel_id} not found")
        return
    ...
    print(f"✅ Successfully mapped Hotelbeds hotel {hotelbeds_hotel_id}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print(f"Mapping single Hotelbeds hotel ID: {single_hotel_id}")
        map_single_hotelbeds_hotel(single_hotel_id)
    else:
        # Process all Hotelbeds hotels from TARGET_COUNTRY
        print("Loading data...")

        hotelbeds_rows = fetch_hotelbeds_rows()
        print(f"📊 Processing {len(hotelbeds_rows)} Hotelbeds {TARGET_COUNTRY} hotels")
        ...
        if not hotelbeds_rows:
            print("No Hotelbeds rows to process")
        else:
            hotelbeds_mapping_cache = fetch_mappings_by_supplier(SUPPLIER_NAME)
            agoda_mapping_cache = fetch_mappings_by_supplier("agoda")
            ittid_generator = IttidGenerator(fetch_max_ittid_sequence_by_country())
            ...
            for i, hb in enumerate(hotelbeds_rows, 1):
                if i % 50 == 0:
                    print(f"Progress: {i}/{len(hotelbeds_rows)}")
                process_one_hotelbeds(
                    hb,
                    geo_index,
                    lat_step,
