...
table_2 = f"s_{SUPPLIER_NAME}_master"
table_3 = "mapping"
...
MAX_TOTAL_SCORE = sum(SCORE_WEIGHTS.values())

# Thresholds tuned for normalized 0..1000 scale
AUTO_MATCH_THRESHOLD = 800
REVIEW_THRESHOLD = 795
...
def fetch_supplier_rows():
    ...
    with engine.begin() as conn:
        return conn.execute(sql, {"country_code": TARGET_COUNTRY}).mappings().all()


def fetch_agoda_candidates(country_code):
    sql = text(
        f"""
        SELECT
            Id,
...
def fetch_mappings_by_supplier(supplier):
    ...
    with engine.begin() as conn:
        rows = conn.execute(sql, {"supplier": supplier}).mappings().all()
        return {row["hotel_id"]: row for row in rows}
...
def insert_supplier_mapping_row(ittid, supplier_hotel_id):
    sql = text(
        f"""
        INSERT INTO {table_3} (
            ittid,
    ...
    with engine.begin() as conn:
        result = conn.execute(
            sql,
            {
                ...
                "hotel_id": supplier_hotel_id,
            },
        )
        return result.lastrowid


def ensure_supplier_mapping(supplier_hotel_id, ittid, supplier_mapping_cache):
    existing = supplier_mapping_cache.get(supplier_hotel_id)
    if existing:
        return existing

    mapping_id = insert_supplier_mapping_row(ittid, supplier_hotel_id)
    record = {
        "Id": mapping_id,
        "ittid": ittid,
        "supplier": SUPPLIER_NAME,
    ...
    supplier_mapping_cache[supplier_hotel_id] = record
    return record
...
def update_supplier_status(supplier_row_id, status_value):
    sql = text(
        f"""
        UPDATE {table_2}
        SET status = :status
...
def get_candidate_pool(supplier_row, geo_index, lat_step):
    lat = supplier_row["lat"]
    lon = supplier_row["lon"]
    if lat is None or lon is None:
        return []
...
def process_one_supplier(
    ...
    geo_index,
    lat_step,
    supplier_mapping_cache,
    agoda_mapping_cache,
    ittid_generator,
):
    if hb["lat"] is None or hb["lon"] is None:
        existing = supplier_mapping_cache.get(hb["hotel_id"])
        if existing:
            ...
            mapping = ensure_supplier_mapping(
                hb["hotel_id"], new_ittid, supplier_mapping_cache
            )
            analysis_payload = build_analysis_payload(
                hb,
                SUPPLIER_NAME,
        ...
        update_supplier_status(hb["Id"], "new-mapping")
        print(f"SKIP: {SUPPLIER_NAME.capitalize()} {hb['hotel_id']} (no lat/lon)")
        return
    ...
    if not best_score:
        ...
        if existing:
            ...
            existing = ensure_supplier_mapping(
                hb["hotel_id"], ittid, supplier_mapping_cache
            )
        ...
        update_supplier_status(hb["Id"], "new-mapping")
        print(
            f"❌ No candidates {SUPPLIER_NAME.capitalize()}:{hb['hotel_id']} | new ittid:{ittid}"
        )
        return

    # Collision check
    agoda_existing = agoda_mapping_cache.get(best_score["agoda_hotel_id"])
    if agoda_existing:
        existing_supplier = supplier_mapping_cache.get(agoda_existing["hotel_id"])
        if existing_supplier and existing_supplier["hotel_id"] != hb["hotel_id"]:
    ...
    if best_score["total_bm"] >= AUTO_MATCH_THRESHOLD:
        agoda_map = agoda_mapping_cache.get(best_score["agoda_hotel_id"])
        matched_ittid = (
            agoda_map["ittid"]
            if agoda_map
        ...
        mapping = ensure_supplier_mapping(
            hb["hotel_id"], matched_ittid, supplier_mapping_cache
        )
        analysis_payload = build_analysis_payload(
            hb,
            SUPPLIER_NAME,
        ...
        update_supplier_status(hb["Id"], "mapped")

        print(
            f"✅ MATCH HB:{hb['hotel_id']} -> Agoda:{best_score['agoda_hotel_id']} "
            f"| ittid:{matched_ittid} | score:{best_score['total_bm']:.2f}/{MAX_TOTAL_SCORE} "
            f"| {best_score['distance_km']:.2f}km"
        )
        return
    ...
    if best_score["total_bm"] >= REVIEW_THRESHOLD:
        ...
        update_supplier_status(hb["Id"], "review")

        print(
            f"🟡 REVIEW HB:{hb['hotel_id']} -> Agoda:{best_score['agoda_hotel_id']} "
            f"| score:{best_score['total_bm']:.2f}/{MAX_TOTAL_SCORE} "
            f"| {best_score['distance_km']:.2f}km"
        )
        return
    ...
    print(
        f"❌ WEAK MATCH HB:{hb['hotel_id']} -> Agoda:{best_score['agoda_hotel_id']} "
        f"| score:{best_score['total_bm']:.2f}/{MAX_TOTAL_SCORE} "
        f"| new ittid:{ittid}"
    )


def fetch_single_supplier_hotel(hotel_id):
    """Fetch a single supplier hotel by hotel_id"""
    sql = text(
        f"""
        SELECT
            Id,
    ...
    with engine.begin() as conn:
        result = conn.execute(sql, {"hotel_id": hotel_id}).mappings().first()
    return result


def map_single_supplier_hotel(supplier_hotel_id):
    """Map a single supplier hotel to Agoda"""
    print(f"Loading single {SUPPLIER_NAME.capitalize()} hotel: {supplier_hotel_id}")

    hb = fetch_single_supplier_hotel(supplier_hotel_id)
    if not hb:
        print(f"❌ {SUPPLIER_NAME.capitalize()} hotel {supplier_hotel_id} not found")
        return

    print(f"Found hotel: {hb['name']} ({hb['hotel_id']}) in {hb['country_code']}")

    agoda_candidates = fetch_agoda_candidates(hb["country_code"])
    print(f"📊 {len(agoda_candidates)} Agoda {hb['country_code']} candidates loaded")

    if not agoda_candidates:
        print("No Agoda candidates found for this country")
        return

    supplier_mapping_cache = fetch_mappings_by_supplier(SUPPLIER_NAME)
    agoda_mapping_cache = fetch_mappings_by_supplier("agoda")
    ittid_generator = IttidGenerator(fetch_max_ittid_sequence_by_country())
    geo_index, lat_step = build_geo_index(agoda_candidates, MATCH_RADIUS_KM)

    print(f"🗺️ Geo index built with lat_step={lat_step:.6f}")
    print(
        f"Score scale: max={MAX_TOTAL_SCORE}, "
        f"auto-match>={AUTO_MATCH_THRESHOLD}, review>={REVIEW_THRESHOLD}"
    )
    ...
    print(
        f"✅ Successfully mapped {SUPPLIER_NAME.capitalize()} hotel {supplier_hotel_id}"
    )
...
if __name__ == "__main__":
    ...
    # Check if a single hotel ID is provided as argument
    if len(sys.argv) > 1:
        single_hotel_id = sys.argv[1]
        print(
            f"Mapping single {SUPPLIER_NAME.capitalize()} hotel ID: {single_hotel_id}"
        )
        map_single_supplier_hotel(single_hotel_id)
    else:
        # Process all supplier hotels from TARGET_COUNTRY
        print("Loading data...")

        supplier_rows = fetch_supplier_rows()
        print(
            f"📊 Processing {len(supplier_rows)} {SUPPLIER_NAME.capitalize()} {TARGET_COUNTRY} hotels"
        )

        agoda_candidates = fetch_agoda_candidates(TARGET_COUNTRY)
        print(f"📊 {len(agoda_candidates)} Agoda {TARGET_COUNTRY} candidates loaded")

        if not supplier_rows:
            print(f"No {SUPPLIER_NAME.capitalize()} rows to process")
        else:
            supplier_mapping_cache = fetch_mappings_by_supplier(SUPPLIER_NAME)
            agoda_mapping_cache = fetch_mappings_by_supplier("agoda")
            ittid_generator = IttidGenerator(fetch_max_ittid_sequence_by_country())
            ...
            for i, hb in enumerate(supplier_rows, 1):
                if i % 50 == 0:
                    print(f"Progress: {i}/{len(supplier_rows)}")
                process_one_supplier(
                    hb,
                    geo_index,
                    lat_step,
