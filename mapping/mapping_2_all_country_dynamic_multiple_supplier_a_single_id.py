...
def fetch_all_hotelbeds_rows_to_process():
    sql = text(f"""
        SELECT
            Id,
            hotel_id,
...
def ensure_hotelbeds_mapping(hotelbeds_hotel_id, ittid, hotelbeds_mapping_cache):
    ...
    existing = fetch_mapping_by_supplier_hotel(SUPPLIER_NAME, hotelbeds_hotel_id)
    if existing:
        hotelbeds_mapping_cache[hotelbeds_hotel_id] = existing
        return existing
...
def main():
    ...
    hotelbeds_rows_by_country = fetch_all_hotelbeds_rows_to_process()
    target_countries = sorted(hotelbeds_rows_by_country)
    if not target_countries:
        print(f"❌ No {SUPPLIER_NAME.upper()} country codes found to process")
        return

    total_rows = sum(len(rows) for rows in hotelbeds_rows_by_country.values())
    print(
    ...
    for base_supplier in BASE_SUPPLIERS:
        ...
        for idx, country_code in enumerate(target_countries, 1):
            hotelbeds_rows = [
                row
                for row in hotelbeds_rows_by_country[country_code]
            ...
            print(
                ...
                f"{len(hotelbeds_rows)} remaining {SUPPLIER_NAME.upper()} hotels to match"
            )
            ...
            for i, hb in enumerate(hotelbeds_rows, 1):
                if i % 50 == 0:
                    ...
                    print(
                        f"  ⏳ Progress: {i:4d}/{len(hotelbeds_rows)} ({percent:3d}%)"
                    )

                is_matched = process_one_hotelbeds(
                    hb,
                    base_supplier,
                    geo_index,
                    lat_step,
                ...
                if is_matched:
                    matched_target_hotel_ids.add(hb["hotel_id"])
