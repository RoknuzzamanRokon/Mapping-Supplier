...
def main():
    ...
    if not target_countries:
        print("No Hotelbeds country codes found to process")
        return

    total_rows = count_hotelbeds_rows_to_process()
    print(
        f"Found {len(target_countries)} countries and {total_rows} Hotelbeds rows "
        "pending processing"
    )
    ...
    for idx, country_code in enumerate(target_countries, 1):
        ...
        print(
            f"▶️ Processing country {country_code} ({idx}/{len(target_countries)}) - "
            f"{len(hotelbeds_rows)} Hotelbeds rows"
        )

        if not hotelbeds_rows:
            print(f"Skipping {country_code}: no Hotelbeds rows to process")
            continue

        agoda_candidates = fetch_agoda_candidates(country_code)
        print(f"📊 {len(agoda_candidates)} Agoda {country_code} candidates loaded")

        geo_index, lat_step = build_geo_index(agoda_candidates, MATCH_RADIUS_KM)
        print(f"🗺️ Geo index built for {country_code} with lat_step={lat_step:.6f}")
        print(
        ...
        for i, hb in enumerate(hotelbeds_rows, 1):
            if i % 50 == 0:
                print(f"Progress {country_code}: {i}/{len(hotelbeds_rows)}")
